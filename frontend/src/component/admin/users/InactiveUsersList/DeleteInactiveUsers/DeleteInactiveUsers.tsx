...
export const DeleteInactiveUsers = ({showDialog, closeDialog, inactiveUsersLoading, removeInactiveUsers, inactiveUserApiErrors, inactiveUsers}: IDeleteInactiveUsersProps) => {
    const ref = useLoading(inactiveUsersLoading);
    return (
        <Dialogue open={showDialog} title={`Really delete all inactive users?`}
                  onClose={closeDialog}
            ...
            <div ref={ref}>
                <ConditionallyRender
                    condition={Boolean(inactiveUserApiErrors[DEL_INACTIVE_USERS_ERROR])}
                    show={
                        <Alert
                            data-loading
                            severity='error'
                ...
                <div style={flexRow}>
                    <Typography
                        ...
                        Are you sure you want to delete {inactiveUsers.length === 1 ? `1 inactive user` : `${inactiveUsers.length} inactive users`}
                   </Typography>
                </div>
            </div>
        </Dialogue>
