...
describe('NewFeatureStrategyCreate', () => {
    ...
    // I don't understand why this test is here. What is it doing?
    test.skip('Should update multiple constraints with the correct react key', async () => {
        ...
        const popoverOpenButtons2 = screen.getAllByRole('button', {
            name: 'Add values',
        });

        fireEvent.click(popoverOpenButtons2[0]);

        const popoverInput = screen.getByRole('textbox', {
            name: 'Constraint Value',
        });
        fireEvent.change(popoverInput, {
            target: { value: '666' }, // we add this value, but we never check for it? Why not?
        });
        const addButton = screen.getByRole('button', {
            name: 'Add',
        });
