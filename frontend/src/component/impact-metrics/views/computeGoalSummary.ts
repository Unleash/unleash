...
export type GoalSummary = {
    // The headline number to display. For count/sum modes this is the
    // aggregated total over the window (matches what MultimetricTotals shows
    // for the same metric). For last-value modes (avg / percentiles / rps)
    // this is the most recent data point.
    current: number;
    // Indicates how `current` was derived, so the UI can label it correctly.
    mode: GoalSummaryMode;
    // Movement vs. the start of the window. Only meaningful for `latest` mode
    // — for cumulative totals, comparing first-vs-last bucket value isn't a
    // useful answer to "how is the goal moving?", so we leave deltas null.
