...
// The shared `MultimetricFeatureEvent` describes a single feature's toggle
// events. Impact views aggregate events across many features on one chart, so
// we carry an optional `featureName` to disambiguate which flag a toggle
// belongs to. Kept local (rather than widening the shared type) so the whole
// feature stays co-located and removable. Structurally a superset of the base
// type, so values flow into the shared chart/overlay components unchanged.
