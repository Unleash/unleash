...
it.each([true, false])(
    'respects enterprise status if the item is marked with enterprise = %s',
    (enterprise) => {
...
it.each([
    ...
    'should not display if the current version is %s the `versionLessThan` property',
    (_, version) => {
        const test = shouldBeDisplayed({
            ...defaultShouldBeDisplayedProps,
            version,
...
it.each([true, false])(
    `it should respect the flag's evaluation status when the flag is %s`,
    (flagEnabled) => {
        const test = shouldBeDisplayed({
            ...defaultShouldBeDisplayedProps,
