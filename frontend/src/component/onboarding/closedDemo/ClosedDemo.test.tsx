...
test('plays the kill-switch story in the first step', () => {
    ...
    expect(screen.getByTestId('CLOSED_DEMO_BUG_ALERT')).toBeInTheDocument();

    fireEvent.click(toggle);
    expect(screen.getByTestId('CLOSED_DEMO_BUG_RESOLVED')).toBeInTheDocument();
    vi.useRealTimers();
});
...
test('manages variants and shows a user payload in the variants step', () => {
    ...
    fireEvent.click(screen.getByTestId('CLOSED_DEMO_ADD_VARIANT_BUTTON'));
    ...
    const grid = screen.getByTestId('CLOSED_DEMO_USER_GRID');
    fireEvent.click(within(grid).getAllByRole('button')[0]);
    expect(screen.getByTestId('CLOSED_DEMO_PAYLOAD')).toHaveTextContent(/cta/);
});
