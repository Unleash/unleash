...
const aggregateTotal = (
    ...
    if (SUM_MODES.has(aggregationMode)) {
        return data.reduce((sum, [, v]) => sum + toFiniteNumber(v), 0);
    }
    ...
    const sum = data.reduce((acc, [, v]) => acc + toFiniteNumber(v), 0);
...
const labelFor = (config: ImpactMetricsConfigSchema): string =>
    config.title || config.displayName || config.metricName;
