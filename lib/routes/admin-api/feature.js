...
const variantsSchema = joi.array().unique('name');
...
function validatevariants(featureToggle) {
    return new Promise((resolve, reject) => {
        if (!featureToggle.variants) {
            return resolve(featureToggle);
        }

        let result = joi.validate(featureToggle.variants, variantsSchema);

        if (result.error) {
            let variantsError = result.error;
            if (result.error.message.indexOf('duplicate') > 0) {
                variantsError = new NameExistsError(
                    'variants with duplicate name exist'
                );
            }
            return reject(variantsError);
        }

        featureToggle.variants = result.value;

        featureToggle.variants = featureToggle.variants.map(variantConfig => {
            result = joi.validate(variantConfig, variantSchema);
            if (result.error) {
                throw result.error;
            }
            return result.value;
...
module.exports.router = function(config) {
    ...
    router.put('/:featureName', (req, res) => {
        ...
        featureToggleStore
            ...
            .then(() => validatevariants(updatedFeature))
            .then(() =>
                eventStore.store({
