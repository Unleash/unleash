...
function validateVariations(featureToggle) {
    return new Promise((resolve, reject) => {
        if (featureToggle.variations) {
            if (featureToggle.variations.length < 2) {
                return reject(
                    new ValidationError(
                        'You must define at least two variations'
                    )
                );
            }
            ...
            const variationsResult = joi.validate(
                featureToggle.variations,
                variationsSchema
            );

            if (variationsResult.error) {
                return reject(
                    new NameExistsError('Variations with duplicate name exist')
                );
            }

            featureToggle.variations = variationsResult.value;
        }
