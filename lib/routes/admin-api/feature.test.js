...
test('should require at least two variations when creating a feature toggle', t => {
    t.plan(0);
    const { request, base } = getSetup();
    ...
    return request
        .post(`${base}/api/admin/features`)
        .send({
            name: 'sample.missing.strategy',
            strategies: [{ name: 'default' }],
            variations: [{ name: 'variation1' }],
        })
        .set('Content-Type', 'application/json')
        .expect(400);
});

test('should require at least two variations when updating a feature toggle', t => {
    t.plan(0);
    const { request, featureToggleStore, base } = getSetup();
    featureToggleStore.addFeature({
        name: 'ts',
...
test('should not allow variations with same name when creating feature flag', t => {
    t.plan(0);
    const { request, base } = getSetup();
    ...
    return request
        ...
        .send({
            name: 'ts',
            strategies: [{ name: 'default' }],
            variations: [{ name: 'variation1' }, { name: 'variation1' }],
        })
        .set('Content-Type', 'application/json')
        .expect(403);
});

test('should not allow variations with same name when updating feature flag', t => {
    t.plan(0);
    const { request, featureToggleStore, base } = getSetup();
    featureToggleStore.addFeature({
        name: 'ts',
