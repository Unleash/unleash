...
- Enablement timing for feature_environment.setEnabled with enabled=true: schedule the setEnabled IMMEDIATELY AFTER the FIRST strategy.create for that feature — same minute, just a few seconds later. The sequence should be: create the initial strategy, then enable the flag, THEN begin ramping. Do NOT delay enablement to a later step (e.g., don't wait to enable until after ramp to 10%). Delayed enablement makes the canary silently inactive and surprises operators. The only exception is when the user EXPLICITLY asks to delay enablement (e.g., "don't enable until we hit 50%"). Ordering invariant that the server enforces: the first strategy.create MUST have a fireAt strictly earlier than any setEnabled(true); enabling an environment with no strategies auto-creates a 100% default rollout. The executor ticks every minute, so both actions scheduled inside the same minute will fire in the correct order within that tick.
