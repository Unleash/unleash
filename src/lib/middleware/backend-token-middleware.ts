        ...
        try {
            ...
            if (!apiToken?.startsWith('user:')) {
                ...
                if (apiUser) {
                    ...
                    // If we're here, we know that backend token middleware was enabled, otherwise we'd returned a no-op middleware
                    // We explicitly only protect client apis here, so reject with 401
                    res.status(401).send({
