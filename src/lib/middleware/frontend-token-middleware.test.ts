...
import frontendApiAccessMiddleware from './frontend-token-middleware.js';
...
test('should not do anything if request does not contain a authorization', async () => {
    ...
    const func = frontendApiAccessMiddleware(config, { apiTokenService });
...
test('should set user if known token', async () => {
...
])('should not set user if not /api/client with token type %s', async (type) => {
    expect.assertions(5);
...
*/
test('should not set user if disabled', async () => {
    const apiUser = new ApiUser({
        tokenName: 'default',
        permissions: [CLIENT],
        project: ALL,
    ...
    const func = frontendApiAccessMiddleware(disabledConfig, { apiTokenService });
