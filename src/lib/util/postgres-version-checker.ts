...
const MIN_SUPPORTED_POSTGRES_VERSION: SemVer = semver.parse('15.0.0')!;
...
export async function compareAndLogPostgresVersion(
    ...
    if (pgSemVer !== null && lt(pgSemVer, MIN_SUPPORTED_POSTGRES_VERSION)) {
        logger.error(
            `You are running an unsupported version of PostgreSQL: ${postgresVersion}. You'll have to upgrade to Postgres 15 or newer to continue getting our support.`,
        );
    } else {
