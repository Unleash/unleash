...
const REPORT_TXT = path.resolve(
    process.cwd(),
    process.env.STARTUP_PROFILER_TXT ?? 'startup-profiler-report.txt',
);
const TRACE_JSON = path.resolve(
    process.cwd(),
    process.env.STARTUP_PROFILER_TRACE ?? 'startup-profiler-trace.json',
);
const CPU_PROFILE = path.resolve(
    process.cwd(),
    process.env.STARTUP_CPU_PROFILE ?? 'startup-profiler.cpuprofile',
);
