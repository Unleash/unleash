...
describe('segments in change requests', () => {
    const CR_ID = 123456;
    let user;
    ...
    test.each(['updated', 'added'])(
        'should not delete segments that are in active change requests (%s strategy with segment)',
        async (action) => {
            await createSegment({ name: 'a', constraints: [] });
            const featureFlag = mockFeatureToggle();
            await createFeatureToggle(app, featureFlag);
            const [segment] = await fetchSegments();

            await createCR('In Review');
    ...
    test.each(['Rejected', 'Applied', 'Cancelled'])(
        'should not prevent deletion of segments used in %s change requests',
        async (state) => {
            await createSegment({ name: 'a', constraints: [] });
            const featureFlag = mockFeatureToggle();
            await createFeatureToggle(app, featureFlag);
