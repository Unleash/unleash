...
describe('flag name validation', () => {
    test('should validate names if project has flag name pattern', async () => {
        ...
        const result = await projectService.validateFeatureNames(projectId, [
            ...validFeatures,
            ...invalidFeatures,
        ]);
        ...
        expect(result).toMatchObject({
            state: 'invalid',
            mismatchedNames: invalidFeatures,
            patternData: featureNaming,
        });

        const validResult = await projectService.validateFeatureNames(
            projectId,
            validFeatures,
        );
    ...
    test.each([null, undefined, ''])(
        ...
        async (pattern) => {
            ...
            const features = ['a', 'b'];
            const result = await projectService.validateFeatureNames(
                projectId,
                features,
            );
