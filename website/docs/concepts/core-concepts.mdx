...
-   [API access tokens](/concepts/api-tokens-and-client-keys)
-   [Projects](/concepts/projects)
-   [Segments](/concepts/segments)
-   [Strategy types](/concepts/activation-strategies)
-   [Tag types](/concepts/feature-flags#tags)
-   [Unleash context](/concepts/unleash-context) fields (including [custom context fields](/concepts/unleash-context#custom-context-fields))
-   Users, [user groups](/concepts/rbac#user-groups) and [roles](/concepts/rbac)

## Projects

[**Projects**](/concepts/projects) contain [feature flags](/concepts/feature-flags) and their configurations, and a set of active [environments](/concepts/environments).
...
Environments are adjacent to [feature flags](/concepts/feature-flags) in Unleash: neither one contains the other, but they come together to let you define activation strategies.
...
[**Feature flags**](/concepts/feature-flags) are at the heart of Unleash’s functionality. Feature flags belong to projects and live next to project environments. In and of itself, a feature flag doesn’t do anything. You must assign [**activation strategies**](/concepts/activation-strategies) to it for it to start taking effect.

When creating a feature flag, you must assign a unique (across your Unleash instance) name, a [feature flag type](/concepts/feature-flags#feature-flag-types), a [project](/concepts/projects) it belongs to, and an optional description. Everything except for the name can be changed later.

![A project containing an environment containing a feature flag configuration](/img/anatomy-of-unleash-features.png)

## Activation strategies

[**Activation strategies**](/concepts/activation-strategies) (or just **strategies** for short) are the part of feature flags that tell Unleash **who should get a feature**. An activation strategy is assigned to **one **feature flag in **one **environment.

![An environment containing a feature flag configuration with an activation strategy](/img/anatomy-of-unleash-strategy.png)

When you check a [feature flag](/concepts/feature-flags) in an application, the following decides the result:
...
[**Strategy constraints**](/concepts/activation-strategies#constraints) (or just **constraints**) help you fine-tune your strategies. They are an extra layer of prerequisites that help you narrow the audience of a strategy down. Strategy constraints are applied to [**activation strategies**](/concepts/activation-strategies).
...
You can define constraints on whatever properties you want in your [Unleash context](/concepts/unleash-context).

Constraints are applied to individual strategies and do not stay in sync with each other. When you need to have the same constraints applied to multiple strategies and need those constraints to stay in sync, use [**segments**](/concepts/segments).

![Example of constraints applied to strategies](/img/anatomy-of-unleash-constraint.png)

## Segments

[**Segments**](/concepts/segments) add extra functionality on top of [**strategy constraints**](/concepts/activation-strategies#constraints). A segment is a reusable collection of strategy constraints with a name and an optional description. When you apply a segment to a [strategy](/concepts/activation-strategies), the strategy will be evaluated as if all of the segment's constraints were applied to it.
...
[Release templates](/concepts/release-templates) provide a way to standardize how you roll out features across your application. While activation strategies define who gets a feature, release templates define how that access expands over time.
...
By default, a [feature flag](/concepts/feature-flags) in Unleash only tells you whether a feature is enabled or disabled, but you can also add more information to your flags by using [**feature flag variants**](/concepts/feature-flag-variants). Variants also allow you to run [A/B testing experiments](/guides/a-b-testing).
...
You configure an [Unleash SDK for your server](/sdks) to communicate with Unleash. When rendering the page, you check the state of the new-color-scheme feature and render a different stylesheet based on the results.

In pseudocode (loosely based on the [Node.js SDK](/sdks/node)), that might look like this:
