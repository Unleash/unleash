...
**Custom activation strategies** let you define your own activation strategies to use with Unleash. When the [built-in activation strategies](/concepts/activation-strategies) aren't enough, custom activation strategies are there to provide you with the flexibility you need.
...
The exact method for implementing custom strategies will vary between SDKs, but the backend SDKs follow the same patterns. For frontend SDKs ([Android](/sdks/android), [JavaScript](/sdks/javascript-browser), [React](/sdks/react), [iOS](/sdks/ios), [Flutter](/sdks/flutter)), the custom activation strategy must be implemented in [Unleash Edge](/unleash-edge).
