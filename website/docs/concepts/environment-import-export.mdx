...
Api admin state is deprecated from version 5 and removed from version 6. We recommend using the new [Environment Import & Export](/concepts/import-export).
...
The api endpoint `/api/admin/state/export` will export feature flags and strategies as json by default.
...
| featureToggles | `true` | Include feature flags in the exported data |
...
For example if you want to download just feature flags as yaml:
...
You can import feature flags and strategies by POSTing to the `/api/admin/state/import` endpoint (keep in mind this will require authentication).\
