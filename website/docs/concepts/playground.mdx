...
The [Unleash context](/concepts/unleash-context) represents a client SDK's configuration and used for evaluating your features. You can construct your own context or use a JSON version of a context from a client.
...
-   **`currentTime`**: The `currentTime` property of the [Unleash context](/concepts/unleash-context) gets auto-populated with the current time. You can override this manually by providing a value of your own.
...
Because you can add multiple values for each context field, each feature-environment cell contains the number of combinations that evaluated to `true` and `false` for the feature in the given environment. This can be expanded into a more detailed overview over what combinations of context fields evaluated to `true` and `false` along with any [variants](/concepts/feature-flag-variants).

![Playground results in development](/img/playground-results-development.png)

As with all of Unleash's client SDKs, the playground respects [stickiness](/concepts/stickiness). The [stickiness algorithm](/concepts/stickiness#calculation) guarantees that you'll always get the same variants and the same gradual rollout results if you provide the same context, as long as you provide the context field used for calculating stickiness.
...
2. The [strategy is the 'Application Hostname' strategy](#hostname-strategy).

Even if the playground doesn't recognize or otherwise can't evaluate a specific strategy, it _may_ still evaluate the overall strategy result to `false` (and be certain that it is the right result). If a strategy has [constraints](/concepts/activation-strategies#constraints) or [segments](segments) that are not satisfied, the playground knows that the strategy result wouldn't be `true`, regardless of the actual strategy implementation. As such, if a strategy can't be evaluated, it can be **either** _unknown_ or _`false`_.

### Custom Strategies

The playground does not have any implementations for [custom strategies](/concepts/activation-strategies#custom-strategies). As a result, custom strategies do not evaluate to `true` regarless of the context.

### Hostname strategy

The [application hostname strategy](/concepts/activation-strategies#hosts) is a little different from the other built-in strategies: it depends on external state and _not_ on the Unleash context to evaluate. In short, the strategy checks its application environment to get the hostname, so the Unleash context has no effect on the result of this strategy.
