...
A segment is a reusable collection of [strategy constraints](/concepts/activation-strategies#constraints). [Feature flags](/concepts/feature-flags) have one or more [activation strategies](/concepts/activation-strategies) that define how and when a feature is shown, for example, to a percentage of users, to a specific group, or only in certain environments.
 
Segments make these strategies easier to manage. Instead of repeating the same rules in multiple places, you can define a segment once and reuse it across any number of feature flags or [release templates](/concepts/release-templates).
...
Before you begin, make sure you have [permissions](/concepts/rbac) to manage segments in your project. If your project requires [change requests](/concepts/change-requests#change-requests-for-segments), your changes to segments need to go through the same approval process before they go live.
...
You can also add segments to strategies defined inside [release templates](/concepts/release-templates).
...
If a segment is referenced in an open [change requests](/concepts/change-requests), first edit, apply, or reject that change request.
...
Before you begin, make sure you that have [permissions](/concepts/rbac) to manage segments in your project. If your project requires [change requests](/concepts/change-requests#change-requests-for-segments), deleting segments also need to go through the same approval process.
...
If you're self-hosting, you can [configure segment limits](/concepts/resource-limits) with environment variables.
