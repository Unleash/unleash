...
**Version**: `5.4+` and [SDK compatibility](/sdks#feature-compatibility-in-backend-sdks).
...
Variants are configured within the feature flag's [activation strategies](/concepts/activation-strategies).
...
[Stickiness](/concepts/stickiness) is evaluated based on [context data](/concepts/unleash-context) and it ensures that the same user consistently sees the same variant. If no context data is provided, the traffic is spread randomly for each request.
...
2. Select your desired [strategy type](/concepts/activation-strategies#strategy-types) and click **Configure**.
...
[Environment-level variants](/concepts/feature-flag-variants) are deprecated. Use strategy variants instead.