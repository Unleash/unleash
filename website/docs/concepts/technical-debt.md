...
pagination_next: concepts/insights
---
...
A feature flag can have one of the following states: _active_, _potentially stale_, or _stale_. Unleash marks all flags as potentially stale automatically once they pass their [expected lifetime](/concepts/feature-flags#feature-flag-types). This gives you an indication of when to review and clean up a feature flag in code.
...
Marking a flag as stale generates a `feature-stale-on` [event](/concepts/events#feature-stale-on). You can use [an integration](/integrate) to trigger automated workflows, such as posting notifications in a Slack channel, breaking project builds if the code contains stale flags, or automatically opening pull requests to remove stale flags from the code.
...
Each project has a **Project status** dashboard, where you can view its technical debt rating—the percentage of healthy flags compared to stale or potentially stale flags. To keep your project's technical debt low, [archive stale feature flags](/concepts/feature-flags#archive-a-feature-flag) and remove them from your codebase. To view your project's technical debt rating over time, go to [Analytics](/concepts/insights).
