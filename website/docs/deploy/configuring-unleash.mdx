...
| `INIT_BACKEND_API_TOKENS`        | N/A             | Comma-separated list of [Backend tokens](/concepts/api-tokens-and-client-keys#backend-tokens) to create on first startup (if no tokens exist in the database).                   |
| `INIT_FRONTEND_API_TOKENS`       | N/A             | Comma-separated list of [Frontend tokens](/concepts/api-tokens-and-client-keys#frontend-tokens) to create on first startup (if no tokens exist in the database).                 |
...
To learn more about managing users, [implementing single sign-on](/concepts/sso) instead of passwords, setting up access controls, and using audit logs, read the [User Management, Access Controls and Auditing guide](/guides/user-management-access-controls).
