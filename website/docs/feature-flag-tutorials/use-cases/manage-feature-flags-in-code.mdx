...
![Evaluate flags multiple times](/img/evaluate-flags-in-many-places.jpg)
...
The first line of defense against flag debt is clarity. A flag named `temp_fix_v2` is a mystery waiting to happen. A [good name](/reference/feature-toggles#set-a-naming-pattern) provides immediate context about the flag's purpose and owner, both in the Unleash UI and in your code.
...
- Use [external links](/reference/feature-toggles#external-links): Instead of putting an issue number like JIRA-376 in the name, use the links feature in Unleash to connect the flag directly to the corresponding Jira ticket, GitHub issue, or design document.
...
### Unit tests
At the unit level, the focus is narrow: does this specific piece of code work correctly when its controlling flag is on and when it's off? For any unit of code affected by a flag, you should have tests that cover both states:
...
For broader tests that cover a single service (component) or the entire application (E2E), you need a strategy to handle the growing number of flags. Here are some common approaches:
- **Baseline state** = all new off: This is your most important test suite. It runs with all feature flags turned off, simulating your stable production environment. It answers the question: "Has adding new, dormant flag-gated code caused regressions in existing functionality?"
- **New features state** = all flags on: This suite runs with all (or most) feature flags enabled. Its purpose is to catch unexpected, negative interactions between multiple new features that might be developed in parallel. It helps ensure that the application is coherent in its most feature-rich state.
