...
[SDKs](/reference/sdks) embedded in your applications fetch feature flag configurations from the Unleash server or [Unleash Edge](/reference/unleash-edge).
...
SDKs cache the last known valid configuration. If the Unleash API server or Unleash Edge becomes temporarily unavailable, the SDK continues operating using these cached flags and predefined default values, ensuring your application remains functional and resilient to network interruptions or upstream service issues.
...
- **How it works**: You deploy, manage, and scale all Unleash components (Unleash API server, database, Admin UI, Unleash Edge) within your own infrastructure (cloud or on-premises).
...
- **Scaling and infrastructure**: You are responsible for architecting for high availability (for example, Multi-AZ API deployment, database clusters), scaling compute resources, database capacity planning, backups, disaster recovery strategies, monitoring, and security updates.
...
- **Ideal for**: Teams needing the operational ease of a managed core platform but requiring absolute control over where end-user context is evaluated (for example, for data privacy, residency regulations) by keeping Edge within their network boundaries.
...
The database layer must be performant and highly available. Use managed database services (such as AWS RDS for PostgreSQL) configured for high availability.
...
#### Horizontal scaling
We run Unleash API instances across multiple AWS Availability Zones (Multi-AZ) in each region. Load balancers distribute incoming traffic, and auto-scaling groups automatically adjust the number of instances based on observed load.
...
<details>
  <summary>Recommended specification</summary>
    ...
    The resource requirements for your Unleash server instances depend on factors like the number of connected SDKs, the frequency of flag evaluations, and the overall traffic volume.
...
For organizations with globally distributed users and a high number of client-side flag evaluations (in web applications and mobile apps), relying solely on the Unleash API server can introduce latency for end-users and place significant read load on the API server and database.
...
- **Configure your SDKs**: Set SDKs to connect to the Unleash server via the load balancer endpoint. Consider deploying highly-available Unleash Edge instances as intermediaries. Edge caches flags locally, improving performance and resilience.
...
<details>
  <summary>Self-hosted observability</summary>
    ...
    - **Health checks**: Poll `/health` endpoints on API and Edge instances. Monitor `/internal-backstage/prometheus` on Edge.
...
- **Global Edge network**: Unleash Edge distributes flag serving globally for low latency and high throughput.
...
Unleash Edge is a lightweight, globally distributable, read-only cache for your feature flag configurations. Think of it as a specialized distribution layer for feature flags.
...
- **Unleash Edge**: Using Unleash Edge provides an additional layer of availability, serving flags even if the Unleash API server is down.
...
- **Unleash API**: The API server can scale horizontally using ASGs. Its capacity depends on the instance sizes and number of instances deployed.
