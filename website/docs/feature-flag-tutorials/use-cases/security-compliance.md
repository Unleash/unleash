...
Edge allows you to decide what you expose to the world. You only have to expose one project (or a predefined set of projects) and environments through Edge. Nothing else will be exposed. In our cloud, if this is combined with an [IP Allow List](#set-up-ip-allow-lists-for-enhanced-security) for the Unleash instance and Hosted Edge, this means you are reducing the attack surface significantly.
