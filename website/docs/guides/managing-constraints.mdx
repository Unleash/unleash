...
Unleash offers several ways to limit feature exposure to a specified audience, such as the [User IDs strategy](/concepts/predefined-strategy-types#userids), [strategy constraints](/concepts/activation-strategies#constraints), and [segments](/concepts/segments). Each of these options makes it easy to add some sort of user identifier to the list of users who should get access to a feature.
...
The first thing to think about would be whether you can group these special cases in some way. Maybe you already have an [Unleash context](/concepts/unleash-context) field that covers the same amount of users. In that case, you can constrain that instead. If you don't have a context field that matches these users, then you might need to create one.
...
[^1]: Well, except for in the case of the [front-end API](/concepts/front-end-api). But even then, the size of the data to transmit matters.
