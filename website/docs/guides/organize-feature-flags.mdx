...
A single [feature flag](/concepts/feature-flags) is defined once within a [project](/concepts/projects), but its configuration (its on/off state and activation strategies) can be different in each [environment](/concepts/environments). This separation allows you to manage a feature's entire lifecycle safely and efficiently.
...
- **Type**: One of four predefined options: `development`, `test`, `pre-production`, or `production`. This environment type is important for [flag lifecycle](/concepts/feature-flags#feature-flag-lifecycle), helping you improve your workflows and flag cleanup.
...
- **Understand technical debt at a glance**: Each project has its own [dashboard](/concepts/technical-debt#project-status) that gives you an instant view of its health, including metrics, recent activity within the project, and information on project-specific resources.
- **Define specific settings**: You can configure settings at the project level, such as a default activation strategy or requiring [approvals for changes](/concepts/change-requests) in production environments.
- **Manage collaboration and visibility**: Projects can be set to [open, protected, or private](/concepts/project-collaboration-mode), defining who can access the project or submit change requests.
...
Remember that your initial project and environment structure is not set in stone. Unleash is flexible. You can add new environments, create new projects, and even move flags between them later. For larger migrations, you can use the [Import/Export](/concepts/import-export) functionality or the [Admin API](/api).
...
Once you have a logical structure for your projects and environments, you can secure it using Unleash's [role-based access control](/concepts/rbac) (RBAC) framework. RBAC allows you to implement the principle of least privilege, ensuring users only have the access they need.
...
A key capability for enterprise governance is the ability to define [custom project roles](/concepts/rbac#custom-project-roles) with [environment-specific permissions](/concepts/rbac#root-permissions). This allows you to precisely control what actions a user can take in each environment.
...
For large organizations, manually managing user accounts and permissions isn't scalable. Unleash integrates with your existing identity provider (like Okta, Microsoft Entra ID, or Keycloak) via SAML or OpenID Connect for [single sign-on](/concepts/sso) (SSO).

Furthermore, Unleash supports the [SCIM protocol](/concepts/scim), which automates the provisioning and de-provisioning of users and the syncing of user groups.
...
Security is enforced through [API tokens](/concepts/api-tokens-and-client-keys) that are scoped to a specific environment and one or more projects. This allows you to apply the principle of least privilege to your applications.
...
### Strategy variants
[Strategy variants](/concepts/strategy-variants), used for A/B testing, are configured as part of an activation strategy within a specific environment.
...
To ensure consistency in how you roll out features, you can use [release templates](/concepts/release-templates).
...
Unleash allows you to create [parent-child dependencies](/concepts/feature-flags#feature-flag-dependencies) between flags, ensuring a child flag can only be enabled if its parent is also enabled.
...
Unleash Enterprise allows you to [clone an entire environment](/concepts/environments#clone-an-environment), which duplicates the environment along with all its feature flag configurations. This is useful for setting up new environments quickly, but it also enables advanced workflows.
...
To ensure these conventions are followed, Unleash allows you to [enforce a specific naming pattern](/concepts/feature-flags#set-a-naming-pattern) at the project level.
...
[Tags](/concepts/feature-flags#tags) are free-form labels that you can apply to feature flags to add flexible, cross-project metadata. 
...
For critical environments like production, enable [change requests](/concepts/change-requests) to enforce a "four-eyes principle", where one user proposes a change and another must approve it. This is essential for compliance and preventing costly mistakes.

Equally important is tracking what happens to your flags. In Unleash, [Event Log](/concepts/events) provides accountability, simplifies debugging, and is indispensable for security and compliance.

### Archive and clean up stale flags

Ensure that your teams have a clear and [effective process](/guides/manage-feature-flags-in-code#flag-cleanup-best-practices) for cleaning up stale feature flags to avoid accumulating [technical debt](/concepts/technical-debt).
