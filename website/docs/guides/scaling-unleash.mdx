...
[SDKs](/sdks) embedded in your applications fetch [feature flag](/concepts/feature-flags) configurations from the Unleash server or [Unleash Edge](/unleash-edge).

[Backend SDKs](/sdks#backend-sdks) perform flag evaluations locally within the application's memory. This eliminates network latency for each evaluation, resulting in extremely fast checks.

[Frontend SDKs](/sdks#frontend-sdks) rely on Unleash Edge as a fast, lightweight caching and evaluation layer closer to the end-user.
...
- **How it works**: You deploy, manage, and scale all Unleash components (Unleash API server, database, [Admin UI](/get-started/unleash-overview#the-unleash-admin-ui), [Unleash Edge](/unleash-edge)) within your own infrastructure (cloud or on-premises).
...
- **How it works**: You use the managed Unleash Cloud for the core platform (API server, database, [Admin UI](/get-started/unleash-overview#the-unleash-admin-ui)) but deploy and manage [Unleash Edge](/unleash-edge) instances yourself within your own network perimeter. SDKs connect to your self-hosted Edge.
...
<details>
  ...
  When self-hosting, we recommend that you implement the following:
    - Auto-scaling for your Unleash API and [Unleash Edge](/unleash-edge) instances based on relevant metrics (CPU, memory, request latency).
...
<details>
  <summary>Recommended specification</summary>
    ...
    The resource requirements for your Unleash server instances depend on factors like the number of connected [SDKs](/sdks), the frequency of flag evaluations, and the overall traffic volume.
...
For organizations with globally distributed users and a high number of [frontend](/sdks#frontend-sdks) flag evaluations (in web applications and mobile apps), relying solely on the Unleash API server can introduce latency for end-users and place significant read load on the API server and database.

[Unleash Edge](/unleash-edge) acts as a lightweight, globally distributed, read-only caching layer for your feature flag configurations. SDKs connect to the nearest Edge instance instead of directly to the Unleash API server.
...
- [SDKs](/sdks) automatically connect to the nearest available region. Traffic fails over to other regions if one region becomes unavailable.
...
- **Configure your SDKs**: Set [SDKs](/sdks) to connect to the Unleash server via the load balancer endpoint. Consider deploying highly-available Unleash Edge instances as intermediaries. Edge caches flags locally, improving performance and resilience.
...
Use the [Connected Edges dashboard](/concepts/network-view#connected-edges) to get observability metrics for Unleash Edge instances, such as ID, region, resource usage, and latency.
...
Unleash achieves speed primarily through how feature flag evaluation works in the [SDKs](/sdks).
...
- **Global Edge network**: [Unleash Edge](/unleash-edge) distributes flag serving globally for low latency and high throughput with a low memory footprint.
...
[Unleash Edge](/unleash-edge) is a lightweight, globally distributable, read-only cache for your feature flag configurations. Think of it as a specialized distribution layer for feature flags.
...
No, your application will not stop working. The Unleash [SDKs](/sdks) are designed for resilience:
...
- **Unleash Edge**: Using [Unleash Edge](/unleash-edge) provides an additional layer of availability, serving flags even if the Unleash API server is down.
...
- **Backend SDKs**: Flags are evaluated locally in your application, scaling as your application does. A single small Unleash instance can support 7,500 connected [SDKs](/sdks), enabling a **750 trillion evaluations per day** across those connected applications. You can scale further by adding more Unleash instances.
- **Frontend SDKs with Unleash Edge**: [Unleash Edge](/unleash-edge) handles flag evaluations and scales horizontally. Performance tests showed 5 Edge instances handling over 11,000 requests per second (RPS), and a single instance configuration capable of managing at least 864 million requests daily.