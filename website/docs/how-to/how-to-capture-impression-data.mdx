...
import VarHandler from '@site/src/components/VariableHandler'

import { useUserData as vars } from '@site/src/theme/Root';

<code>api/{JSON.stringify(vars().userData)}/thing</code>
...
<VarHandler variables={[
    {
        ...
        description: 'The name of the feature toggle to use in requests.',
        default: '<feature-toggle-name>',
    },
    {
        name: 'projectId',
        description: 'The ID of the project to act on',
        default: '<project-id>',
    },
    {
        name: 'apiToken',
        description: 'The API token to use in requests',
        default: '<API-token>',
    },
    {
        name: 'unleashUrl',
        description: 'The URL your unleash instance is hosted on.',
        default: '<unleash-url>',
    },
]}/>
...
<ApiRequest verb="post" payload={{name: vars().userData.featureToggleName || '<feature-toggle-name>', impressionData: true}} url={`api/admin/projects/${vars().userData.projectId || '<project-id>'}/features`} title="Create a feature toggle with impression data enabled."/>
...
<ApiRequest verb="patch" payload={[{op: "replace", path: "/impressionData", value: true}]} url={`api/admin/projects/${vars().userData.projectId || '<project-id>'}/features/${vars().userData.featureToggleName || '<feature-toggle-name>'}`} title="Enable impression data on an existing toggle."/>
