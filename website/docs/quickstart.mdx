...
This pulls the `unleashorg/unleash-enterprise` Docker image and uses the Enterprise Docker Compose file to configure the Unleash server and its database.
...
Request a license by signing up for a free download of our self-hosted trial [here](https://www.getunleash.io/pricing). Once you've signed up, you'll receive an email from Unleash containing your trial license key.

Go to http://localhost:4242/admin/license and install your Unleash Enterprise license trial:

> Note: Without a valid license, your instance is in read-only mode.
