...
Personal API tokens do not have either the the project or environment parts, since they mimic the user that created them. In place of the project field, the string `user` is present so that they can be identified as a personal access token.
