...
Both types use [the same format](#format) but have different intended uses. API tokens are considered to be _secrets_ and should _not_ be exposed to end users.
...
-   Enable the [direct access API](./frontend-api.mdx) and create a [front-end token](#front-end-tokens).
-   Configure an [Unleash Proxy](../sdks/unleash-proxy.md) and use [Proxy client keys](#proxy-client-keys).

### Front-end tokens

**Front-end tokens** are used with [front-end SDKs](../sdks/index.md#front-end-sdks) when used with the [direct access API](./frontend-api.mdx). They grant the user permission to:
