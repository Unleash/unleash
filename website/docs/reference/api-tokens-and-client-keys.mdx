...
Use API tokens to connect to the Unleash server API. API tokens come in fours distinct types:
...
| Type | What kind of token it is: admin, client, user (personal api token) or front-end. |
| Projects | What projects a token has access to - not present in personal api tokens. |
| Environment | What environment the token has access to - not present in personal api tokens. |
...
Do **not** use personal api tokens for:
...
Personal API tokens do not have either the project or environment parts, since they mimic the user that created them. In place of the project field, the string `user` is present so that the token can be identified as a personal access token.
...
-   A token with access to toggles in the "development" environment of all projects:
    ...
    *:development.be44368985f7fb3237c584ef86f3d6bdada42ddbd63a019d26955178
    ```
-   A personal access token with access to everything the user that created them has:
    ```
