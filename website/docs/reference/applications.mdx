...
An **application** is any service that sends requests to the [Unleash Client API](/api), the [Unleash Frontend API](/api), the Unleash Admin API, or any other API that Unleash exposes. This includes [Unleash SDKs](/sdks), [Unleash Edge](/reference/unleash-edge), and even the Admin UI.
