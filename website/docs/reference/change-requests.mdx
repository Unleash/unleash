...
## Global environment change requests
...
Global environment change requests let you preconfigure required approvals at the environment level. When configured, all new projects automatically inherit these approval requirements for the specified environments.

Depending on your needs, global environment change requests can be used in two ways:

* **Defaults**: Set global defaults for approvals per environment, but allow project owners and users with [project update permissions](/reference/rbac#projects) to override them for individual projects.
* **Enforcement**: Set mandatory global approval requirements per environment. To fully enforce these global settings, ensure that the project has no owners or users with project update permissions, preventing any modifications at the project level.
