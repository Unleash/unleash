...
Feature flags are the central concept that we built Unleash around. In Unleash, feature flags are organized within [projects](../reference/projects). Feature flags can have different [activation strategies](../reference/activation-strategies) for each of their project's [environments](../reference/environments), and can also be given [variants](../reference/feature-toggle-variants) to facilitate A/B testing.
...
| feature flag type | Yes | Release | The [feature flag's type](..//reference/feature-toggles). |
| project | Yes | The [default project](../reference/projects). When created from a project page in the admin UI, that project will be the default value instead. | The [project](../reference/projects) that should contain the feature flag. |
| description | No | N/A | A description of the feature flag's purpose. |
| enable impression data | Yes | No | Whether to enable [impression data](../reference/impression-data) for this flag or not. |
