...
Feature flags exist within a [project](/reference/projects) and have distinct configurations and [activation strategies](/reference/activation-strategies) for each of the project's active [environments](/reference/environments). Activation strategies determine whether a feature flag is enabled in a specific environment.
...
Feature flags have different [activation strategies](/reference/activation-strategies) for each environment, allowing you to manage feature rollouts per environment.
...
Activation strategies determine whether a feature flag activates for a specific [Unleash context](/reference/unleash-context), such as a particular user or segment. When multiple strategies are in place, the flag activates if any one of the strategies evaluates to true.

### Strategy variants

You can enhance your feature flags with [strategy variants](/reference/strategy-variants). Feature flags let you define who gets access to a feature. With variants, you can also determine which version of the feature they see.
For example, you might use variants to run an A/B testing experiment.

Variants are defined by a name and weight, which determines the likelihood of each variant being shown. [Stickiness](/reference/stickiness) ensures that users consistently see the same variant based on predefined parameters.
...
State gives you an indication of when to [clean up a feature flag in code](/reference/technical-debt).
...
You can use this to signal to your team to stop using the feature in your applications. Stale flags will show as stale in the [project status dashboard](/reference/technical-debt#project-status).

Marking a flag as stale generates the `feature-stale-on` [event](/reference/events#feature-stale-on). You can use [an integration](/integrate) to trigger automated workflows, such as posting notifications in a Slack channel, breaking project builds if the code contains stale flags, or automatically opening pull requests to remove stale flags from the code.
