...
For Jira Data Center, check out the [Jira Server plugin](/reference/integrations/jira-server-plugin-installation)
...
We recommend using a [service account](/reference/service-accounts) token for communicating with Unleash. Service accounts are also required to integrate with [change requests](/reference/change-requests)
