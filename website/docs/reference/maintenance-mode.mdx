...
Unleash maintenance mode is a feature that allows administrators to switch Unleash into a read-only "use-only" mode, with the exception that clients can still send metrics. This feature is particularly useful during important events when any accidental or deliberate changes to the feature/strategy configuration could lead to issues.
...
![Maintenance mode being toggled on](/img/maintenance-configuration.png)
