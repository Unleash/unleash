...
Both the network overview and the network traffic diagrams show you **applications** that have made requests to the Unleash instance. An **application** is defined as anything that sends requests to the [Unleash client API](/reference/api/unleash/client), the [Unleash front-end API](front-end-api),  the Unleash admin API, or any other API that Unleash exposes. This includes [Unleash SDKs](./sdks/index), [Unleash Edge](/docs/generated/unleash-edge), the [Unleash proxy](/docs/generated/unleash-proxy), and even the admin UI.
