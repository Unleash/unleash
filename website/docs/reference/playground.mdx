...
This section describes what information the playground needs to evaluate the context against your existing features.
...
Unleash clients all require an `appName` to start up. If you do not provide an `appName`, the playground will assign a default value to this property instead.

#### Implicit fields

The `currentTime` property of the [Unleash context](../user_guide/unleash-context.md) gets auto-populated with the current time. You can override this manually.
...
As with all of Unleash's client SDKs, the playground respects [stickiness](../advanced/stickiness.md). This guarantees that you'll always get the same variants and the same gradual rollout results if you provide the same context, as long as you provide the context field used for calculating stickiness.
