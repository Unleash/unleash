...
Frontend SDKs are used in frontend and mobile applications. They communicate with Unleash or [Unleash Edge](/reference/unleash-edge) through the [Frontend API](/get-started/api-overview) and require a [frontend token](/reference/api-tokens-and-client-keys#frontend-tokens). Unlike backend SDKs, frontend SDKs do not perform the flag evaluation locally. Instead, they fetch all enabled feature flags for a given [Unleash Context](/reference/unleash-context). The flag evaluation happens either in [Unleash Edge](/reference/unleash-edge), or in the Unleash server, when using the [Frontend API](/get-started/api-overview) directly.
