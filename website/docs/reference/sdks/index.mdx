...
To identify which frontend and backend SDK instances are connected to Unleash, the following three headers must be attached to all requests made by the SDK to the server (`/api/client` for backend SDKs and `/api/frontend` for frontend SDKs):
