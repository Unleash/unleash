...
Segments stay in sync across strategies and any updates propagate automatically to all activation strategies that use them. This makes them ideal for grouping larger releases together, for example, enabling or disabling a collection of flags at the same time using time-based constraints.
...
Make sure you have [permission](/reference/rbac) to manage segments in your project. If your project requires [change requests](/reference/change-requests#change-requests-for-segments), your changes may require approval before they go live.
...
If a segment is referenced in an open [change requests](/reference/change-request), first edit, apply, or reject that change request.
...
For project-scoped segments, if the project uses change requests, the segment deletion needs to go through the same approval process.
