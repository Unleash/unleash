...
## Segment limits
<div id="large-segments"><!-- Redirect old anchor links here --></div>

In theory, you could you create segments with a thousand constraints, each with a million values. But this wouldn't scale well, so there are limitations in place to stop you from doing this. Unleash enforces the following limits on use of segments:

1. A segment can have **at most 100 values** specified across all of its constraints. That means that if you add a constraint that uses 10 values, you will have 90 more values to use for any other constraints you add to the same segment.

2. You can apply at most `strategySegmentsLimit` segments to a single strategy. Different strategies (even on the same feature) do not affect each other.

Furthermore, segments are just constraints, so any limitations that apply to constraints also apply to segments.
