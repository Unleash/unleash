...
- Feature activation strategies can be combined in complex ways. The [Unleash Playground](/reference/playground) is an invaluable tool. You can use an access token along with various context values (input via the UI) to simulate how a flag will be resolved for different users and scenarios, helping you verify your configuration.
...
-   If results are still unexpected, use the [Playground](/reference/playground) to simulate different user contexts and verify that the feature flag and its variants are resolving as intended.
