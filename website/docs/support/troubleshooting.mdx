...
pagination_next: support/availability
---
...
-   Ensure the feature flag has an [activation strategy](/concepts/activation-strategies) associated with it that will evaluate to `true` for your given context.
-   Confirm that the feature flag has been **enabled** in the specific environment your client application is using. (ref: [enabling a feature flag](/concepts/feature-flags))
...
#### Check token type
-   To connect to the Frontend API or Edge, you **must** use a [Front-end API token](/concepts/api-tokens-and-client-keys#frontend-tokens). Other token types will not work.
...
-   When using a **gradual rollout** strategy, pay close attention to the [stickiness](/concepts/stickiness) configuration.
...
- Feature activation strategies can be combined in complex ways. The [Unleash Playground](/concepts/playground) is an invaluable tool. You can use an access token along with various context values (input via the UI) to simulate how a flag will be resolved for different users and scenarios, helping you verify your configuration.
...
-   Next, configure [strategy variants](/concepts/strategy-variants) to represent "enabled" and "disabled" states. You can assign percentages to these variants (e.g., 50% "enabled", 50% "disabled").
...
-   Check the rollout percentage of your [gradual rollout activation strategy](/concepts/activation-strategies). If you intend to include 100% of your user base in the A/B/n test, ensure the rollout percentage is set to 100%.

#### Check stickiness and context
-   Revisit the [stickiness](/concepts/stickiness) configuration.
...
-   Refer to the documentation on [feature flag variants](/concepts/feature-flag-variants).
...
-   For example, using the [Unleash React SDK](/sdks/react), you might follow the [check variants](/sdks/react#check-variants) section. Given the example variants "A" and "B", your code might look like this:
...
-   If results are still unexpected, use the [Playground](/concepts/playground) to simulate different user contexts and verify that the feature flag and its variants are resolving as intended.
...
-   These settings can also be managed via the [Unleash API](/api).
...
When you integrate Unleash with an identity provider (IdP) like [Okta](/provisioning/how-to-setup-provisioning-with-okta) or [Entra ID](/provisioning/how-to-setup-provisioning-with-entra) via [SCIM](/concepts/scim), your IdP becomes the source of truth for user information.
...
By creating a [group](/concepts/rbac#user-groups) in Unleash and assigning it an admin role, and then configuring your IdP to push that group and its membership via SCIM, you get fully automated permission management:

1. In the Unleash Admin UI, go to **Admin settins > User config > Groups** and create a new group, for example `Unleash Admins`.
2. Assign the Admin role or your desired [custom-root role](/concepts/rbac#custom-root-roles) to this group.
...
If you run into an issue when [configuring your Network dashboards](/concepts/network-view#configure-a-data-source), verify that you've set up the `PROMETHEUS_API` URL correctly:
