...
Feature flags are intended for dynamic and temporary use, giving you [runtime control](https://docs.getunleash.io/topics/feature-flags/runtime-control) to enable or disable functionality in a live environment. This flexibility supports [short-lived experiments](https://docs.getunleash.io/topics/feature-flags/short-lived-feature-flags), such as A/B testing or gradual rollouts, instead of relying on configuration settings that require an application redeploy to take effect. Despite the ease of turning off and on features or functionality with feature flags, they should not be repurposed for managing configuration.
...
This guide was all about using feature flags at scale. For similar tips on building and scaling the feature flag system itself, see our [feature flag best practices guide](https://docs.getunleash.io/topics/feature-flags/feature-flag-best-practices).
