...
-   We do allow short spikes in traffic and our adaptive infrastructure will automatically scale to your needs.
...
-   If you have scaling challenges, it could be worth it to have a look at [Unleash Edge](../reference/unleash-edge)
...
Unleash no longer hosts instances of the proxy, but makes the [Frontend API](../reference/front-end-api) available to all Pro and Enterprise customers. The API is backed by an Amazon RDS database. Your applications can connect to the frontend API from your own cloud or other hosting solution.
