...
When Unleash hosts the proxy, it'll run alongside the Unleash API, admin UI, and the backing database within Unleash's Kubernetes setup in AWS. Your applications can connect to the proxy from your own cloud or other hosting solution. You'll need to configure your proxy SDKs with the following details:
