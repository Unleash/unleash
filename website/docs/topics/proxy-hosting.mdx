...
Edge will fetch feature toggles from the specified upstream Unleash instance for every client API key it has been made aware of, either during bootstrapping (recommended) or separate endpoint requests. It will then periodically sync features with upstream.
