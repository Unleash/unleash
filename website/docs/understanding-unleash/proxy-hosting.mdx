...
![An architecture diagram of using the proxy in a multi-region, self-hosted setup.](/img/hosting-self-hosted-multi-region.png)
...
| **Client-side context processing** | Processed on Unleash-managed Edge infrastructure, but does not reach the central Unleash server or database | Processed entirely within your infrastructure. PII does not leave your network boundary |
