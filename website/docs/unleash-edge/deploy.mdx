...
When running in [Edge mode](#modes-of-operation), the instance requires at least one valid [backend token](/concepts/api-tokens-and-client-keys#backend-tokens) at startup to connect to the upstream Unleash instance.
...
Edge enforces strict scope validation based on the startup token. It refuses requests from SDKs that attempt to access [projects](/concepts/projects) or [environments](/concepts/environments) outside the scope of the startup token.

Incoming requests must match the environment and project access specified in the initial tokens. For example, if your startup token grants access to `project-a` and `project-b` in development, an SDK request for `project-c` in development or any [projects](/concepts/projects) in production will be rejected.
...
- **[Backend SDKs](/sdks#backend-sdks)**: Use `CLIENT_TOKENS` (or `--client-tokens`). These tokens grant access to the `/api/client/features` endpoint.
- **[Frontend SDKs](/sdks#frontend-sdks)**: Use `FRONTEND_TOKENS` (or `--frontend-tokens`). These tokens grant access to the `/api/frontend` and `/api/proxy` endpoints.
...
Pre-trusted tokens allow you to bypass upstream validation for specific frontend tokens. This is useful for migrating off the old Unleash Proxy.
...
If a frontend token requests data that is not currently cached, Edge returns `HTTP 511 Network Authentication Required`. The response body will indicate which project and environment requires a valid backend token to hydrate the cache.
