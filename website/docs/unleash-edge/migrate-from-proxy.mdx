...
If you are an **Unleash Hosted** customer moving to the managed Enterprise Edge, you do not need to deploy any infrastructure.

1. **Request Enterprise Edge**: Contact your customer success reprentative to request Enterprise Edge.
...
Edge dynamically routes requests to the correct environment based on the API token provided by the SDK.
...
Edge supports a dedicated [Offline mode](/unleash-edge/configure#offline-mode) for local development or air-gapped environments. This replaces the need to run a local Proxy with a mock config.
