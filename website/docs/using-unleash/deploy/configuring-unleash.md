...
- **authentication** - (object) - An object for configuring/implementing custom admin authentication
    ...
    - `initialAdminEmailUser`: `{ name: string, email: string } | undefined` - The initial admin name and email. Takes precedence over `initialAdminUser`. Can be set using the `UNLEASH_DEFAULT_ADMIN_NAME` and `UNLEASH_DEFAULT_ADMIN_EMAIL` environment variables.
