...
This guide explains how to configure your self-hosted Unleash instance when running it via [Docker Compose or the Docker CLI](/understanding-unleash/deploy/getting-started).
...
<TabItem value="docker-cli-iss" label="Unleash OSS">
...
| `DATABASE_SSL`                     | N/A           | Enables/configures SSL. Expects a stringified JSON object with SSL options.  |
...
| `EMAIL_SENDER`       | `noreply@unleash-hosted.com` | The email address used as the sender; update accordingly. |
